...
const type: ElementsType = "DateRangeField";
...
const extraAttributes: {
  ...
  helperText: "Escolha o intervalo de datas",
  required: false,
  conditions: [],
};
      ...
      <DatePickerWithRange
        ...
        defaultDate={
          value.start_date && value.end_date
            ? { from: new Date(value.start_date), to: new Date(value.end_date) }
            : undefined
        }
        modal={submitValue ? false : true}
        disabled={disabled}
        setDateRange={(date: DateRange | undefined) => {
          if (date && typeof date === "object") {
            setValue(date);
            if (!submitValue) return;
            const value = JSON.stringify(date);
            const errors = DateRangeFieldQuestionnaireElement.validate(
              element,
              value,
            );
            setErrors(errors);
...
const PropertiesComponent = ({
  ...
  function applyChanges(values: z.infer<typeof propertiesSchema>) {
    ...
    toast({
      title: "Sucesso",
      description: "Intervalo de Datas atualizado com sucesso",
    });
...
export const DateRangeFieldQuestionnaireElement: QuestionnaireElement = {
  type,
  construct: (id: string) => ({
    id,
    type,
