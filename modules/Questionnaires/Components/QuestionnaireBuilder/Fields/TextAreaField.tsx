...
const QuestionnaireComponent = ({
  ...
  const element = elementInstance as CustomInstance;
  const { label, helperText, required, placeHolder, rows, conditions } =
  ...
  const updateValue = useCallback(
    (newValue: string) => {
      ...
      const errors = TextAreaFieldQuestionnaireElement.validate(
        element,
        newValue,
      );
      setErrors(errors);
...
const PropertiesComponent = ({
  ...
  function applyChanges(values: z.infer<typeof propertiesSchema>) {
    ...
    toast({
      title: "Sucesso",
      description: "Campo de texto longo atualizado com sucesso",
    });
...
export const TextAreaFieldQuestionnaireElement: QuestionnaireElement = {
  type,
  construct: (id: string) => ({
    id,
    type,
